...
- **Watermark 기록**: 토픽별 처리 상태를 `di_ops.cdc_watermark` 테이블에 append
...
spark-submit --py-files utils.zip kafka_to_iceberg_stream.py \
  --dag-id "my_stream" --service <service> \
  --topics "prefix.schema.table1,..." \
...
3. **orphan 파일 삭제**: 지정된 스키마의 `remove_orphan_files` 실행
